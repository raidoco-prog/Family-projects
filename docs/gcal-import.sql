...
--
--  כל מה שכאן מסומן בשמך, ולכן גלוי לך ולסטס בלבד — הילדים
--  לא רואים את זה. התורים מסומנים דרך patient_id, והטיפוח דרך
--  event_participants בסוף החלק. ראה visibility.sql לכלל עצמו.
--
--  התנאי לכך הוא ש-visibility.sql רץ על המסד. בלעדיו האפליקציה
--  עדיין משתפת הכל עם כולם, וכל מה שכאן יהיה גלוי גם ליונתן
--  ולשני. הרץ אותו קודם.
-- ============================================================
...
-- הטיפוח מסומן בשמך במפורש. תור רפואי מסתתר מעצמו — יש לו
-- patient_id — אבל «מספרה» הוא אירוע general רגיל, וללא הסימון
-- הזה הוא נחשב «של כל הבית» ולכן גלוי לילדים.
insert into event_participants (event_id, member_id)
select e.id, (select id from gcal_owner)
from events e
where e.external_key in (
  'gcal:vdu20o1m3ocun1qdvq7e7951tg',   -- תספורת וצבע
  'gcal:sabovgvqcq0fcpbcjp0pii21uo',   -- לייזר
  'gcal:_651k2h2174pj2ba46p14ab9k'     -- מספרה
)
on conflict do nothing;
